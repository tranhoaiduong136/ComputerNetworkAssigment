	...
	def processRtspRequest(self, data):
			...
			if self.state == self.INIT:
				...
				try:
					for f in filename.split(','):
						print(f)
						self.clientInfo['videoStream'][f]=VideoStream(f)
		...
		# Process PLAY request 		
			...
			if self.state == self.READY:
				...
				if self.clientInfo["rtpSocket"]==None:
	...
	def sendRtp(self, filename):
		...
		while True:
			...
			if self.clientInfo['event'].isSet(): 
				break 
			data = self.clientInfo['videoStream'][filename].nextFrame(self.BACKWARDING)
			if data: 
				frameNumber = self.clientInfo['videoStream'][filename].frameNbr()
				try:
